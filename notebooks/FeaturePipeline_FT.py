...
# Importing RobustScaler from sklearn.preprocessing
...
output_1_df = spark.createDataFrame(df1)
...
spark.sql(f"desc formatted {output_path}").filter(F.col("col_name") == "Location").select("data_type").collect()[0][0]
